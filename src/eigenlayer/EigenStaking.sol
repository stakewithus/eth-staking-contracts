...
contract EigenStaking is IEigenStaking, IStakingEvents, ReentrancyGuard, Owned, Pausable, StakingConstants {
    ...
    /// @notice Mapping of users to EigenUser contracts which users manage their EigenPods and collect rewards from.
    ...
    function stake(address user_, DepositData[] memory data_) external onlyOperator {
        ...
        EigenUser eigenUser = EigenUser(registry[user_]);
        eigenUser.stake{value: _DEPOSIT_AMOUNT}(data_);

        bytes[] memory pubkeys = new bytes[](length);

        for (uint256 i = 0; i < length; ++i) {
            pubkeys[i] = data_[i].pubkey;
        }
